...
//            });
            ...
            submitLoginForm(user, null, token);
        } else {
            var user = $("#username").val();
...
function submitLoginForm(username, password, token){
    var form = document.getElementById('login-form');
    if (!form) {
        var validateHtml = '';
        if ( password && !token)
            validateHtml = '<input type="hidden" name="password" id="login-password" value="' + password + '" />';
        else
            validateHtml = '<input type="hidden" name="token" id="login-token" value="' + token + '" />';
        $form = $('<form action="login" method="post" id="live-form" target="_self">' +
            '<input type="hidden" name="username" id="login-username" value="' + username + '" />' +
            validateHtml +
            '</form>');
        $('body').append($form);
    } else {
